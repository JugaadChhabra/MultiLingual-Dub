...
from services.retry import (
    free_translate_max_attempts,
    free_translate_rate_limiter,
    retry_call,
)

FREE_TRANSLATE_LANGUAGES = {"fr", "de", "es", "ru", "pt"}

# Shared across every free-translate call in the process, so our own
# concurrency (languages fanned out per row, rows in a batch) can't burst past
# Google's per-IP quota. Configured in services/retry.py, where process-tuning
# env knobs live.
_LIMITER = free_translate_rate_limiter()
...
def translate_text_free(
    ...
    *,
    ...
    return retry_call(
        ...
        max_attempts=free_translate_max_attempts(),
    )
